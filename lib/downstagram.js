 ...
 * @param {Boolean} options.keepMetadata, whether or not to write the metadata for each photo
 * @param {String} options.outputDirectory, override output directory
 * @param {Boolean} options.showProgressBar, whether to show progress bar or not
 */
var Downstagram = function(username, options){
    this.userID = 0;
    this.totalPhotos = 0;
    this.username = username;
    this.keepMetadata = options.keepMetadata;
...
Downstagram.prototype.fetchPhotos = function(maxID){
    ...
    instagram.users.recent(recentOptions);
    return;
};

Downstagram.prototype.getFolderPath = function() {
    var folderPath = path.join(this.outputDirectory, this.username + '_instagram_photos');
